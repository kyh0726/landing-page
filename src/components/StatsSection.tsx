  ...
  return (
    <div ref={sectionRef} className="mb-20">
      ...
      <div className="text-center mb-16">
        <h2 className="text-3xl font-display text-gray-900 mb-4 hover:text-purple-700 transition-colors duration-300">
          이미 많은 개발자들이 생산성을 향상시키고 있어요
        </h2>
        <p className="text-lg text-gray-700 mb-8 hover:text-gray-800 transition-colors duration-300 font-medium">
          실제 사용자 데이터로 검증된 효과
        </p>
          ...
          {stats.map((stat, index) => (
            <Card key={index} className="border-2 border-purple-200 bg-white/90 backdrop-blur hover:bg-white hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer group">
              <CardContent className="p-6 text-center">
                ...
                <div className="text-base text-gray-600 group-hover:text-gray-700 transition-colors duration-200 font-medium">{stat.label}</div>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        {testimonials.map((testimonial, index) => (
          <Card key={index} className="border-2 border-blue-200 bg-white/90 backdrop-blur hover:bg-white hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer group">
            <CardContent className="p-8">
              ...
              <p className="text-gray-700 group-hover:text-gray-800 transition-colors duration-200 text-base leading-relaxed font-medium">
                "{testimonial.content}"
              </p>
              
              <div className="mt-4 pt-4 border-t border-gray-200">
                <Badge className="bg-green-100 text-green-800 hover:bg-green-200 transition-colors duration-200 font-medium">
                  ✅ 인증된 사용자
                </Badge>
              </div>
            </CardContent>
      ...
      <div className="text-center mt-12">
        <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl p-8 border border-purple-100">
          ...
          <p className="text-lg text-gray-700 hover:text-gray-800 transition-colors duration-300 font-medium">
